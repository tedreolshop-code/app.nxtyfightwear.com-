...
export const AttendanceModule: React.FC<AttendanceModuleProps> = ({ isAdmin, lockedEmployee, assistingAdmin, openLocationScannerSignal }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {showLocationScanner && <div className="fixed inset-0 z-50 bg-black/70 p-4 flex items-center justify-center no-print"><div className="bg-white rounded-2xl w-full max-w-md max-h-[92dvh] overflow-y-auto overscroll-contain p-5 space-y-4"><div className="flex justify-between"><div><h3 className="font-black">Scan QR Lokasi</h3><p className="text-xs text-gray-500">Arahkan kamera ke QR yang ditempel di pabrik.</p></div><button onClick={() => setShowLocationScanner(false)} className="cursor-pointer"><X className="w-4 h-4" /></button></div><EmployeeQrScanner onScan={handleLocationQr} /></div></div>}
