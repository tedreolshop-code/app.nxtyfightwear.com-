...
export const ProductionHandoffPanel: React.FC<Props> = ({ jobs, currentEmployee, isAdmin }) => {
  ...
  // Serah-terima lama belum menyimpan varian; ambilkan dari pekerjaan produksinya
  // supaya ukuran tetap terlihat tanpa perlu memperbaiki data lama satu per satu.
  const varianDari = (handoff: ProductionHandoff): string =>
    handoff.variant || jobs.find(job => job.id === handoff.job_id)?.variant || '';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        {queuePool.map(item => <div key={item.id} className="bg-white border border-sky-200 rounded-lg p-3 text-xs flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="font-bold text-gray-900 break-words">{item.product_name}{varianDari(item) && <span className="ml-1.5 px-1.5 py-0.5 rounded bg-sky-50 border border-sky-200 text-[10px] font-black text-sky-800">{varianDari(item)}</span>}</p>
